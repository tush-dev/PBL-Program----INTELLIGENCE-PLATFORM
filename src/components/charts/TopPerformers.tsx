        ...
        {data.map((item, i) => (
          <div
            ...
            className={cn(
              ...
              i === 0 ? "bg-amber-50 border border-amber-200" : "bg-slate-50 hover:bg-slate-100"
            )}
          >
            <span className={cn(
    ...
    <Card>
      ...
      <CardContent className="space-y-1.5">
        {data.map((item, i) => (
          <div
            key={item.name}
            className="flex items-center gap-3 px-3 py-2 rounded-lg bg-red-50/50 hover:bg-red-50 transition-colors"
          >
            <span className="w-6 h-6 rounded-full bg-red-200 text-red-700 flex items-center justify-center text-xs font-bold shrink-0">
              {i + 1}
            </span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-900 truncate">{item.name}</p>
